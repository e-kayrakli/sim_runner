...
class ParamRadiosity(Parameter):
    valid_values = ['test']

class BenchmarkRadiosity(Benchmark):
    ...
    def __init__(self):
        self.parameters = [ParamRadiosity()]
        super().__init__()

    def get_checkpointname(self, platform):
        ht = platform.get_ht(self)
        if ht['ParamCores'] == 16:
            check = 'radiosity_%(ParamCores)dp_warm2' % ht
        else:
            check = 'radiosity-%(ParamRadiosity)s_%(ParamCores)dp_warm2' % ht
        return check

    def get_cachename(self, platform):
        ht = platform.get_ht(self)
        return 'radiosity-%(ParamCores)dp-2M_caches.gz' % ht

    def get_ht(self, platform):
        ht = self.get_ht_based_on_vector()
        ht.update( platform.get_ht_based_on_vector() )
        ht['trace'] = 'radiosity_%(ParamRadiosity)s_MOESI_SMP_dir2_%(ParamCores)dp.trace' % ht
        return ht
